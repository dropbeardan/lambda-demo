...
```bash
sam-updated$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-updated$ sam local start-api
sam-updated$ curl http://localhost:3000/
```
...
```bash
sam-updated$ sam logs -n HelloWorldFunction --stack-name sam-updated --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-updated
```
